...
The plan: build something deliberately narrower than n8n (no 500 connectors, no arbitrary code) but deeper on Cekat's own data — same drag-and-drop canvas clients already know, fewer ways to shoot themselves in the foot.
...
## Should have — v1, first milestone (8 weeks)

These are committed for the first working version of the engine.
...
| Scheduled trigger                               | Cron-style — e.g. "every day at 9am"                                                                                                      |
| Event trigger                                   | Message received, label added/removed, CRM record created/changed                                                                         |
| Conversation-inactivity trigger ("chat delay")  | Fires when a conversation has gone quiet for N minutes/hours                                                                              |
...
| Wait/delay step, with a re-check after the wait | Pause before acting, then re-verify the filter still holds before actually doing anything — this is the "post cron filter" from the notes |
...
## Should have — full release, not the first milestone

Committed for v1 overall, but sequenced after the first 8-week milestone.
...
**Flow control:**
* Wait (with re-check on resume)
* Condition/Router

**Actions — first milestone:**
* Add/remove label
* Call webhook
...
Notes on how this differs from the v1 node list above: Ticketing is a full domain here (mirrors the "Ticket Automation" example workflow) even though only some of its nodes are milestone-1; "Loop over list" and "Data" as a category label are new relative to the plain trigger/filter/action framing earlier in this doc — Data nodes read/transform without an external side effect (Find CRM record, Lookup, Format, AI step), which is why they're tagged separately from Action nodes that do have one. This distinction doesn't change v1 scope, just how the palette is organized for the user.
...
| Conversation inactive for N | Inboxes (multi-select) · Duration (number + unit) · Whose turn (toggle: customer hasn't replied / agent hasn't replied) | `conversation`, `contact`, `last_message`, `elapsed` | **[placeholder]** — this is a reading of "bisa by chat delay," not a confirmed spec. Ask the CEO directly: does he mean "conversation gone quiet" (this), or "wait a fixed time after a chat event" (a different node)? Also **[placeholder]**: needs a scheduler/sweeper mechanism that doesn't exist yet — ask engineering how they'd build it. |
| Schedule (cron)             | Frequency (Daily / Weekly / Custom) · Time of day · Timezone                                                            | `scheduled_at`                                       | **[placeholder]** — whether timezone always defaults to a workspace-level setting or can be overridden per automation isn't confirmed. Ask engineering/design.                                                                                                                                                                                    |
...
| Wait / Delay     | Duration or time-of-day · "re-check conditions on resume" toggle         | —                                                         | **[placeholder]** — exact UX for the re-check isn't finalized: does it happen automatically via the toggle, or does the user have to manually add a Condition node after the Wait? Ask engineering which is easier to build first. **[placeholder]** also whether a "resumed at" timestamp should be exposed downstream. |
...
| Create / Update CRM record  | Board · Record reference (Update only) · Field values                  | **[placeholder]** — likely the created/updated record's ID, but not yet confirmed whether this node returns anything usable downstream                  | None else structurally                                                                                                                                             |
